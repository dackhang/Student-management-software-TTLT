...
namespace QuanLySinhVien
{
    public partial class tk_gv : Form
    {
        ...
        private void LoadData()
        {
            ...
            {
                OpenConnection();
                string strsql = "select tk as [Tên Tài Khoản], mk as [Mật Khẩu] from gv";
                ...
                {
                    Dgv_gv.DataSource = dtset.Tables[0];
                }
                else
                {
        ...
        private void Btn_them_Click(object sender, EventArgs e)
        {
            ...
            {
                OpenConnection();
                string s = "insert into gv (tk, mk) values(@username, @password)";
        ...
        private void Btn_sua_Click(object sender, EventArgs e)
        {
            ...
            {
                OpenConnection();
                string strsql = "update gv set mk = @password where tk = @username";
        ...
        private void Btn_xoa_Click(object sender, EventArgs e)
        {
            ...
            {
                OpenConnection();
                string strsql = "delete from gv where tk = @username";
                SqlCommand cmd = new SqlCommand(strsql, myconn);
                cmd.Parameters.AddWithValue("@username", Tbx_ten.Text);
                cmd.ExecuteNonQuery();
        ...
        private void Dgv_gv_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex >= 0)
                {
                    Tbx_ten.Text = Dgv_gv.Rows[e.RowIndex].Cells[0].Value.ToString();
                    Tbx_mk.Text = Dgv_gv.Rows[e.RowIndex].Cells[1].Value.ToString();
                }
            }
            catch (Exception ex)
            {
